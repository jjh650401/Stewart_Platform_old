我進行了測試，現在已經有測試值計算出來了，但是對於 Heave、以及三個旋轉角度自由度的值看起來很奇怪，目前我們要顯示的自由度值該該是由平台 H 高度來進行自由度的計算，所以它應該是一個 +/- 值的方式顯示才對，但是 Heave 值卻是顯示平台最低和最高時的值，另外是旋轉角度自由度的值都太大了，這應該表示單位轉換造成的，請再查查看。對於計算時程式內部所使用的可以統一，只針對輸入和顯示部份要依工程上的習慣再進行轉換，這樣不會造成錯誤的單位轉換。
底下是這次測試時終端的訊息也一併提供你參考，希望能協助找到出錯的原因：
(.venv) D:\Python_Programs\Stewart_Platform_old>D:\Python_Programs\Stewart_Platform_old\.venv\Scripts\python.exe d:/Python_Programs/Stewart_Platform_old/main.py
狀態管理器 (StateManager) v1.0 已初始化。
狀態管理器已重置為初始狀態。
動力學計算引擎 (DynamicsEngine) v1.1 (通用版) 已初始化。
核心計算引擎 v13.5 (單點測試診斷版) 已初始化。
[除錯] 核心引擎的完整狀態已重置。
驅動系統計算引擎 (DriveSystemEngine) v4.0 (v2.0 Guide Aligned) 已初始化。
狀態管理器已重置為初始狀態。
[除錯] 核心引擎的完整狀態已重置。
[除錯] 核心引擎的衍生計算參數已重置。
分析引擎 (AnalysisEngine) v4.0 (整合工作空間分析) 已初始化。

==================================================
[偵錯] AnalysisEngine: run_workspace_analysis 已被呼叫，分析開始。
動力學計算引擎 (DynamicsEngine) v1.1 (通用版) 已初始化。
核心計算引擎 v13.5 (單點測試診斷版) 已初始化。
[除錯] 核心引擎的完整狀態已重置。
[偵錯] AnalysisEngine: 正在呼叫 operational 分析...

==================== DIAGNOSTIC: STARTING WORKSPACE ANALYSIS ====================
DIAGNOSTIC: Analyzing 'operational' workspace.
DIAGNOSTIC: Neutral pose for analysis: [   0.            0.         1048.13723656    0.            0.
    0.        ]
======================================================================

工作空間分析執行緒完成，耗時 0.33 秒。
==================================================
...
核心計算引擎 v13.5 (單點測試診斷版) 已初始化。
[除錯] 核心引擎的完整狀態已重置。
[偵錯] AnalysisEngine: 正在呼叫 mechanical 分析...

==================== DIAGNOSTIC: STARTING WORKSPACE ANALYSIS ====================
DIAGNOSTIC: Analyzing 'mechanical' workspace.
DIAGNOSTIC: Neutral pose for analysis: [   0.            0.         1048.13723656    0.            0.        
    0.        ]
======================================================================
...
工作空間分析執行緒完成，耗時 0.33 秒。
==================================================
請記得你剛才已做了 analysis_engine.py 和 kinematics.py 程式的修改，目前的問題是針對新程式修改後測試的問題，請專注在我的問題，並提出你的分析，不要亂幻想。
...
class CoreEngine:
    ...
    def _find_limit(self, dof_index: int, direction: int, neutral_pose: np.ndarray, space_type: str):
        ...
        result = minimize(scaled_objective, x0=scaled_x0, method='SLSQP', constraints=scaled_constraints, bounds=scaled_bounds, 
                          options={'ftol': config.WORKSPACE_SOLVER_TOLERANCE, 'disp': False}) # [註記] 空間分析的日誌保持關閉
    ...
    def _analyze_workspace_6dof(self, space_type):
        ...
        for i, name in enumerate(dof_names):
            ...
            # --- MODIFICATION START ---
            # [註記] 修正：移除此處的 rad2deg 轉換。
            # 引擎統一輸出弧度 (rad)，交由 analysis_widget.py 進行唯一的顯示轉換。
